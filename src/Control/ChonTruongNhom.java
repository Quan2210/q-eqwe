...
public class ChonTruongNhom {
    ...
    public void insertDataLop() throws Exception{
       ...
       db.getConnect();
           String sql1 = "INSERT INTO LOPHOC(MaLop,TenLop,TENHOCPHAN,Khoa,Siso)"+
                   "VALUES ('" + TaoLop.lop.getMaLop() + "','"+ TaoLop.lop.getTenLop() + "','" +
                   TaoLop.lop.getTenHP() + "','"+ TaoLop.lop.getKhoa() + "'," + TaoLop.lop.getSiso() +")";
           db.doSQL(sql1);
           for(int i = 1;i<sonhom + 1 ; i++){
            String sql3 = "INSERT INTO NHOM(MANHOM,MALOP) " +
                    "VALUES("+ i +",'"+ lop.getTenLop()+"')";
            db.doSQL(sql3);
        }
       for(SinhVien sv:list){
            ...
            db.doSQL(sql);
            if(sv.isNhomTruong()){
                String sql4 = "INSERT INTO TAIKHOAN(TAIKHOAN,MATKHAU,QUYEN)"+
